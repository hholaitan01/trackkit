    ...
    <div>
        ...
        <div className="bg-green-500/8 border border-green-500/20 rounded-xl p-4 mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-2 rounded-lg border border-brand/20 text-brand text-xs font-semibold hover:bg-brand/10 transition"
            >
              Copy
            </button>
          </div>
        ...
        {loading ? (
          ...
          keys.map((k) => (
            <div key={k.id} className="grid grid-cols-[1fr_1.5fr_80px_120px_80px] px-5 py-3 border-b border-white/[0.03] hover:bg-white/[0.02] transition text-sm items-center">
              ...
              <button onClick={() => deleteKey(k.id)} className="text-xs text-red-400/50 hover:text-red-400 transition">
                Revoke
              </button>
            </div>
          ))
      ...
      {showCreate && (
        <CreateKeyModal
          ...
          onCreated={(key) => { setShowCreate(false); setNewKey(key); load(); }}
        />
      )}
    </div>
  );
